...
parent: '[[../20251016_204010.137232b5.md]]'
content_id: 45aa6fb69cda4306692c3e3f3d59c61a5ebdd4d1ae48949121af6f3bee1e8da0
---

# response:

Answering prompt 2: Deno tests for `ConstraintMonitor`.

```typescript
// /src/concepts/ConstraintMonitor.test.ts

/**
 * Deno tests for ConstraintMonitorConcept.
 *
 * To run:
 * > deno test -A --env src/concepts/ConstraintMonitor.test.ts
 *
 * Requires environment variables for MongoDB connection:
 * - MONGODB_URL
 * - DB_NAME
 *
 * Scoped Permissions (if not using -A):
 * --allow-net=<mongo_host>,<gemini_api_host_if_real_llm_is_tested>
 * --allow-env=MONGODB_URL,DB_NAME,GEMINI_API_KEY
 */
...
Deno.test("Operational Principle: Happy Path", async () => {
  await withTestDb(async (db, monitor) => {
    ...
    // 4. Generate alerts for a safe route
    ...
    const goodRouteResult = await plannedRoutes.insertOne({
      ...
      transitLegs: [{ startStopId: "stop-main", endStopId: "stop-trailhead" }],
    ...
    assertEquals(alertIds.length, 0, "A safe route should generate no alerts.");
  });
});
...
Deno.test("Scenario: Severe Weather Alert", async () => {
  await withTestDb(async (db, monitor, stubs) => {
    ...
    assert(alertSummary.severity > 60);
  });
});
...
Deno.test("Scenario: With LLM Stub, Severity is Overridden", async () => {
  await withTestDb(async (db, monitor, stubs) => {
    ...
    assertNotEquals(
      ...
      ruleBasedSeverity,
      "LLM should have overridden the default severity",
    );
