...
export async function ensureCollections(db: Db) {
  ...
  // Check each collection individually to avoid potential regex issues
  ...
  for (const name of collectionNames) {
    try {
      ...
      // If listCollections fails, assume collection doesn't exist
      console.warn(`Could not check collection ${name}:`, error);
    }
  }
...
export class ConstraintMonitorConcept {
  ...
  async updateTransitSchedules(source?: string): Promise<string[]> {
    ...
    try {
      ...
      if (e instanceof MongoBulkWriteError && e.code === 11000) {
        throw new ConflictError(`Transit schedule uniqueness constraint failed: ${(e as Error).message}`);
      }
      throw new Error(`Failed to update transit schedules: ${(e as Error).message}`);
    }
  }
  ...
  async generateAlerts(plannedRouteId: string): Promise<string[]> {
    ...
    // Only call LLM if it's explicitly provided and we want to override
    if (this.#llm?.scoreSeverity && this.#llm.scoreSeverity !== undefined) {
        try {
            severity = await this.#llm.scoreSeverity(message);
        } catch (e) {
            console.error("LLM severity scoring failed:", (e as Error).message);
        }
    }
